...
import type { CustomDropdownOption, Placement, Trigger } from './types'
import { getScrollParents } from '@ui/BetterUI/_utils'
import { ref } from 'vue'
...
// 处理菜单项选择
function handleMenuSelect(key: string | number, item: CustomDropdownOption) {
...
function handleContextMenu(event: MouseEvent) {
  ...
  nextTick(() => {
    // 设置右键菜单位置
    containerPosition.top = event.clientY
    containerPosition.left = event.clientX
...
<template>
  <div class="dropdown">
    <!-- 触发元素 -->
    <div
      ref="triggerRef" :data-popup-trigger="currentId" class="content-wrapper" @click="dropdownTargetClick"
      @mouseenter="onOpenDropdown" @mouseleave="onHideDropdown" @contextmenu="handleContextMenu"
    >
      <slot />
    </div>
    ...
    <Teleport v-if="isOpen" to="body">
      <MenuContainer
        ...
        :menu-items="options" @close="toggleMenu(false)" @select="handleMenuSelect" @mouseenter="onKeepMenuOpenStatus"
        @mouseleave="onHideDropdown"
      />
    </Teleport>
  </div>
</template>
