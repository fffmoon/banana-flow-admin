...
export const useAsyncRouteStore = defineStore('asyncRoute', () => {
  ...
  function _generateRoutesAndMount() {
    // 本地静态配置 + 当前存储的后端数据，去重
    const backendIds = new Set(backendMenuList.value.map(i => i.name))
    const uniqueLocal = localRoutesData.filter(i => !backendIds.has(i.name))
    const mergedMenus = [...uniqueLocal, ...backendMenuList.value]
  ...
  async function checkRoutesUpdate() {
    ...
    try {
      ...
      return data.version !== routesVersion.value
    }
    catch (error) {
