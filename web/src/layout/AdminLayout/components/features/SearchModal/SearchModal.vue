...
const allRoutes = computed(() => {
  return asyncRouteStore.routerMenus.flatMap(route => flattenRoutesByBreadcrumb(route))
})
...
function handleSelectItem(item: FlattenedRoute) {
  const findRes = findRouterById(asyncRouteStore.routerMenus, item.meta.id as string)
  if (!findRes) return
  go(findRes.raw.path, {
    successCallback: () => {
      keyword.value = ''
...
function flattenRoutesByBreadcrumb(routes: IRouteItem): FlattenedRoute[] {
  const result: FlattenedRoute[] = []

  // 如果当前路由被隐藏，直接返回空数组（包括其子路由）
  if (routes.meta?.hideBreadcrumb) {
    return result
  }

  // 当前路由本身是否需要加入扁平化列表
  const shouldIncludeCurrent = routes.meta?.title && routes.path && routes.component
  ...
  if (routes.children?.length) {
    for (const child of routes.children) {
      const flattenedChildren = flattenRoutesByBreadcrumb(child)
...
<template>
  <NModal v-model:show="show" :mask-closable="true">
    <NCard content-style="padding: 0;" class="card-box fixed left-50%"
      ...
      <div class="flex flex-col overflow-hidden" :class="isMobile ? 'h-100vh' : ''">
        ...
        <div class="flex p-20px">
          <NInputGroup>
            <NInput ref="inputRef" v-model:value="keyword" :placeholder="placeholderText" clearable autofocus
        ...
        <div class="min-h-0 flex-1 p-20px pt-0">
          ...
          <template v-else>
            <BScrollbar :content-class="{
              'max-h-40vh': !isMobile,
              'h-full': isMobile,
            }">
              <NList hoverable>
                <NListItem v-for="(item, index) in searchResults" :key="item.meta.id" class="cursor-pointer"
