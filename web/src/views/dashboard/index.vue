          ...
          <NCard title="" :bordered="false" class="h-full">
            <div class="h-full flex flex-col justify-between gap-y-[--space-sm]">
              <div class="flex items-center gap-x-[--space-md]">
                <NAvatar object-fit="cover" round :size="68" :src="userStore.getDispalyUserInfo.avatar"
                ...
                <div class="min-w-0 flex-col">
                  <div class="max-w-[240px] text-truncate text-[17px] text-gray-800 font-medium dark:text-gray-200">
                    Hello, {{ userStore.getDispalyUserInfo.nickname }}
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-wrap gap-[--space-md]">
                ...
                <NTag v-for="(tag, idx) in userStore.getDispalyUserInfo.roles" :key="idx" size="small"
        ...
        <NGridItem span="5 400:5 600:3">
          <NCard :title="title" :bordered="false" class="h-full">
            <template #header-extra>
              <a class="text-[15px] text-primary/90 transition-all hover:text-primary"
                href="https://github.com/fffmoon/banana-flow-admin" target="_blank">
                官方文档 →
              </a>
            </template>
            ...
            <div class="text-[15px] text-gray-600/90 leading-7 space-y-4 dark:text-gray-300">
              <div class="flex items-start gap-[--space-md]">
                ...
                <span>基于 Vue3 + Python 的全栈解决方案，零配置接入企业级开发规范</span>
              </div>
              <div class="flex items-start gap-[--space-md]">
            ...
            <footer class="mt-8 flex items-center justify-end gap-[--space-sm]">
              <NButton type="primary" ghost class="font-medium !h-9 !px-[--space-sm]" tag="a"
                href="https://github.com/fffmoon/banana-flow-admin" target="__blank">
                <template #icon>
                  <div class="i-mdi-document text-lg"></div>
                </template>
                开发指南
              </NButton>
              <NButton type="primary" class="font-medium shadow-sm !h-9 !px-[--space-sm]" tag="a"
                href="https://github.com/fffmoon/banana-flow-admin" target="__blank">
                <template #icon>
          ...
          <NCard title="🎯 下一步目标方向" :bordered="false" class="h-full shadow-sm">
            <div
              ...
              <div v-for="(item, index) in targets" :key="index" class="group flex items-start gap-[--space-md]">
                <div class="mt-2 text-lg transition-transform group-hover:rotate-45" :class="`${item.icon}`"
          ...
          <NCard title="🛠️ 技术栈分布" :bordered="false" class="h-full">
            <div class="h-full flex flex-col">
              <div class="h-[240px] sm:h-[280px]">
                <BetterEcharts :theme="themeStore.getIsDarkTheme ? 'dark' : 'light'"
