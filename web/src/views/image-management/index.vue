...
function openNewDirectory() {
  const currentDirName = breadcrumbs.value.find(c => c.id === currentDirId.value)?.name
  createDirModalRef.value?.toggle(true, { currentDirId: currentDirId.value, currentDirName })
}
...
const columns: DataTableColumns<IDirectoryItem> = [
  ...
  { title: '名称', key: 'name', minWidth: 200 },
  {
    ...
    key: 'type',
    width: 80,
    render: (row) => h(NTag, { type: 'info', size: 'small' }, { default: () => row.type === 'folder' ? '文件夹' : '文件' }), align: 'center'
  },
...
function handleTableAction(type: IFileActionType, rowData: IDirectoryItem) {
  console.log('handleTableAction', rowData.id)
  switch (type) {
    case 'open':
      // 处理打开逻辑
  ...
  <BSpace vertical class="wh-full">
    <NCard class="title">
      <NForm class="flex flex-wrap gap-y-[--space-md]" inline label-width="auto" :show-feedback="false"
        ...
        <NFormItem label="名称">
          <NInput v-model:value="formData.searchName" placeholder="输入名称" :disabled="isLoading"
    ...
    <NCard class="body flex-1">
      <BSpace vertical class="wh-full">
        ...
        <NAlert class="h-32px w-full flex items-center rounded-[var(--custom-border-radius)] px-8px" type="info"
        ...
        <div class="flex-1 overflow-hidden">
          <NDataTable v-if="!isError" :columns="columns" :data="dataList" :row-props="rowProps" :pagination="pagination"
            :loading="isLoading" :bordered="false" :style="{ height: `100%` }" remote flex-height />
          <!-- 数据加载失败 -->
