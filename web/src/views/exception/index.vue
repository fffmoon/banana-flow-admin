 ...
 * - showHomeBtn: 是否显示返回首页按钮 (true/false)
 * - showRetryBtn: 是否显示重新尝试按钮 (true/false)
 * - showLoginBtn: 是否显示重新登录按钮 (true/false)
 */
...
const showHomeBtn = computed(() => {
  const customShow = route.query.showHomeBtn as string
  if (customShow !== undefined) {
    return customShow === 'true'
  }
  return currentConfig.value.buttons.home
})

const showRetryBtn = computed(() => {
  const customShow = route.query.showRetryBtn as string
  if (customShow !== undefined) {
    return customShow === 'true'
  }
  return currentConfig.value.buttons.retry
})

const showLoginBtn = computed(() => {
  const customShow = route.query.showLoginBtn as string
  if (customShow !== undefined) {
    return customShow === 'true'
  }
  return currentConfig.value.buttons.login
})
    ...
    <NResult :status="currentConfig.status" :title="currentConfig.title" :description="customMessage" size="large">
      <template #footer>
        <div class="flex justify-center gap-4">
          <!-- 返回首页按钮 -->
          <NButton v-if="showHomeBtn" @click="handleBackHome">
            返回首页
          </NButton>

          <!-- 重新尝试按钮 -->
          <NButton v-if="showRetryBtn" type="primary" @click="handleRetry">
            {{ redirectPath ? '重新尝试' : '返回上一页' }}
          </NButton>

          <!-- 重新登录按钮 -->
          <NButton v-if="showLoginBtn" type="warning" @click="handleLogout">
            重新登录
          </NButton>
        </div>
      </template>
